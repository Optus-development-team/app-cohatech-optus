...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      if (config.exponential) {
        blurValue = Number(pow(2, progress * 4)) * 0.0325 * currentStrength;
      } else {
      ...
      const overlayColor = 'rgba(10, 10, 20, 0.72)';

      const divStyle: CSSProperties = {
        maskImage: `linear-gradient(${direction}, ${gradient})`,
        WebkitMaskImage: `linear-gradient(${direction}, ${gradient})`,
        backgroundImage: `linear-gradient(${direction}, ${overlayColor} 0%, rgba(10, 10, 20, 0.5) 50%, rgba(10, 10, 20, 0) 100%)`,
        backgroundColor: 'rgba(10, 10, 20, 0.22)',
        backdropFilter: `blur(${blurValue.toFixed(3)}rem)`,
        opacity: Math.min(1, config.opacity),
        transition:
  ...
  const [edgeFactor, setEdgeFactor] = useState<number>(1);
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const baseStyle: CSSProperties = {
      ...
      opacity: (isVisible ? Math.min(1, config.opacity) : 0) * edgeFactor,
  ...
  useEffect(() => {
    ...
    const handle = () => {
      ...
      const fromBottom = Math.max(0, docH - (scrollY + winH));

      const topPad = config.edgePaddingTop ?? 80;
      const bottomPad = config.edgePaddingBottom ?? 80;
      const fade = config.fadeDistance ?? 160;

      const calcFactor = (distance: number) => {
        if (distance >= fade) return 1;
        return Math.max(0, distance / fade);
      };

      const topDistance = Math.max(0, fromTop - topPad);
      const botDistance = Math.max(0, fromBottom - bottomPad);

      const topFactor = calcFactor(topDistance);
      const botFactor = calcFactor(botDistance);
      const factor = Math.min(topFactor, botFactor);

      setEdgeFactor(prev => (Math.abs(prev - factor) > 0.01 ? factor : prev));
    };
